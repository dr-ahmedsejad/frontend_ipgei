...
export default function PremierAccesPage() {
  // router non utilisé : hard navigation au succès pour que le layout dashboard
  // se re-mount et recharge le user (sinon doit_changer_mdp stale dans le state).
  useRouter();
  ...
  const [success,    setSuccess]    = useState<{ matricule: string } | null>(null);
  ...
  const firstLoginMut = useMutation({
    ...
    onSuccess: (res) => {
      if (user) {
        storeUser({ ...user, doit_changer_mdp: false, username: res.nouveau_username });
      }
      setSuccess({ matricule: res.nouveau_username });
    },
      ...
      <div className="min-h-[60vh] flex items-center justify-center p-6">
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-8 max-w-md w-full text-center space-y-5">
          ...
          <p className="text-sm text-slate-600">
            Votre compte est maintenant sécurisé. Votre nouvel identifiant de connexion est :
          </p>
          <div className="bg-[#006633]/5 border border-[#006633]/20 rounded-lg px-4 py-3">
            <p className="text-lg font-bold text-[#006633]">{success.matricule}</p>
            <p className="text-xs text-slate-500 mt-1">Utilisez ce matricule pour vos prochaines connexions.</p>
          </div>
          <button
            onClick={() => { window.location.href = '/dashboard/portail'; }}
