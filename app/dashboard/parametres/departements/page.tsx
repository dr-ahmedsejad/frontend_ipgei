...
import { ArrowLeft, Building, Plus, Pencil, Trash2, X, Loader2, CheckCircle, Search } from 'lucide-react';
...
type DeptForm = {
  ...
  decalage_impair:     string;
  decalage_pair:       string;
};
const EMPTY: DeptForm = { nom: '', code: '', description: '', niveau: '', annee_universitaire: '', decalage_impair: '0', decalage_pair: '0' };
...
export default function DepartementsPage() {
  ...
  const { data, isLoading, error: queryError } = useDepartementsList({ page, search });
  ...
  // Reset à page 1 quand la recherche change
  ...
  const handleSave = () => {
    ...
    if (editing) update.mutate({ id: editing.id, input: payload as never }, { onSuccess, onError });
    else         create.mutate(payload as never, { onSuccess, onError });
  };
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <h1 className="text-xl font-bold text-iss-dark">Départements</h1>
          </div>
          <p className="text-sm text-iss-gray">{count} département{count !== 1 ? 's' : ''} au total</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 overflow-hidden">
        {loading ? (
          <table className="data-table">
            <thead><tr><th>Code</th><th>Nom</th><th>Niveau</th><th>Année</th><th>Actions</th></tr></thead>
            <tbody>
              {[1,2,3].map(i => (
          ...
          <div className="flex flex-col items-center justify-center p-12 text-center">
            ...
            <p className="text-sm font-semibold text-iss-dark mb-1">Aucun département enregistré</p>
            <p className="text-xs text-iss-gray mb-4">Ajoutez les départements de l&apos;établissement.</p>
            <button onClick={openAdd}
