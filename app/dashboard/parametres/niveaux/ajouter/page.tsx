...
import { useNiveauxMutations } from '@/lib/api/niveaux-hooks';
...
export default function AjouterNiveauPage() {
  const router = useRouter();
  const [niveau, setNiveau] = useState('');
  const [error, setError] = useState<string | null>(null);
  const { create } = useNiveauxMutations();
  const saving = create.isPending;

  const handleSave = () => {
    if (!niveau.trim()) { setError('Ce champ est requis.'); return; }
    setError(null);
    create.mutate({ niveau }, {
      onSuccess: () => router.push('/dashboard/parametres/niveaux'),
  ...
  return (
    <div className="space-y-6 max-w-xl">
      <div className="flex items-center gap-3">
        <Link href="/dashboard/parametres/niveaux"
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-iss-dark">Nouveau niveau</h1>
        </div>
      </div>
