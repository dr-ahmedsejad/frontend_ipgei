...
import { MapPin, Plus, Pencil, Trash2, X, Loader2, CheckCircle, Search } from 'lucide-react';
...
export default function SallesPage() {
  ...
  const handleSave = () => {
    ...
    const onSuccess = () => {
      closeForm();
      showToast(editing ? 'Salle modifiée' : 'Salle ajoutée');
    };
  ...
  const handleDelete = () => {
    ...
    remove.mutate(toDelete.id, {
      onSuccess: () => { showToast('Salle supprimée'); setToDelete(null); },
      onError:   (e) => { alert(e instanceof Error ? e.message : 'Erreur'); setToDelete(null); },
    });
  };
        ...
        <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-6"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" min={0} value={form.capacite}
                onChange={e => set('capacite', e.target.value)} className={INPUT} />
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 text-center">
            ...
            <p className="text-sm font-semibold text-iss-dark mb-1">
              {search ? 'Aucun résultat' : 'Aucune salle enregistrée'}
            </p>
            {!search && (
              <><p className="text-xs text-iss-gray mb-4">Ajoutez les salles et amphithéâtres disponibles.</p>
              <button onClick={openAdd}
